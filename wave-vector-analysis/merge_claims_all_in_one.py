...
def build_front_matter(results_dir: Path, present, front_pdf: Path):
    ...
    with PdfPages(str(front_pdf)) as pdf:
        ...
        fig.text(0.5, 0.58,
                 dt.datetime.now().strftime("Generated %Y-%m-%d %H:%M"),
        ...
        y -= 0.03
        note = (
            "Note: each component PDF is generated by its own script against the "
            "current spark_tracks / vector_clusters CSVs and the Box 'Calcium "
            "videos' + 'Calcium claims.docx' sources. Dataset hygiene (single "
            "canonical tracks/clusters pair) is still pending; see chat notes."
        )
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        build_front_matter(results_dir, present, front_pdf)
